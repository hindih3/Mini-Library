...
static void BM_push_back(benchmark::State& state) {
    ...
    for (auto _ : state) {                  // the framework decides how many reps
        ...
        benchmark::DoNotOptimize(v.data()); // this is your sink(), built in
        benchmark::ClobberMemory();
    }
    state.SetItemsProcessed(state.iterations() * n);  // lets it report items/sec
}
...
BENCHMARK_MAIN();   // generates main() for you